...
struct Vertex {
  ...
  getAttributeDescription() {
    std::array<VkVertexInputAttributeDescription, 2> attriuteDescriptions{};
    // position attribute
...
private:
  ...
  void createImageViews() {
    ...
    for (size_t i = 0; i < this->m_swapChainImageViews.size(); ++i) {
      ...
      // multiple image views for each image representing the views for the left
      ...
      if (vkCreateImageView(this->m_device, &createInfo, nullptr,
                            &(this->m_swapChainImageViews[i])) != VK_SUCCESS) {
  ...
  void createVertexBuffer() {
    ...
    // associate the allocated memory with the vertex buffer
